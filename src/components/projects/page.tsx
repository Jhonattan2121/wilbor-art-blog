...
async function getHivePosts(username: string) {
  try {
    ...
    console.log('Posts found:', response.originalPosts.length);

    const formattedPosts = response.originalPosts.flatMap(post => {
    ...
    const uniquePosts = formattedPosts.reduce((acc: Photo[], post: Photo) => {
      const key = `${post.author}-${post.permlink}-${post.src}`;

      // If post doesn't exist or is a more recent video, add/update
      const existingPost = acc.find((p: Photo) => `${p.author}-${p.permlink}-${p.src}` === key);
      if (!existingPost) {
        acc.push(post);
      }
    ...
    return {
      ...
      originalPosts: response.originalPosts,
    };
  } catch (error) {
...
export default async function GridPage() {
  ...
  const loggedInUsername = cookieStore.get('hive_username')?.value;
  ...
  console.log('Logged in user:', loggedInUsername); // Debug log

  const posts = formattedPosts.map(post => {
    const isAuthor = loggedInUsername === post.author;
    console.log(`Post ${post.permlink} - Author: ${post.author}, LoggedIn: ${loggedInUsername}, isAuthor: ${isAuthor}`); // Debug log
